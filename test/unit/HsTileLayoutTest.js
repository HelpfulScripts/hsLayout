...
describe("factory: HsTileLayout", function() {
    ...
    describe('should support default size', function() {
    ...
    describe('should support hs-size', function() {
        beforeEach(inject(function($compile, $rootScope) {
            scope = $rootScope.$new();
            scope.num = [1,2,3];
            elem = '<hs-layout hs-fill-last-col><hs-widget hs-pos=\'["25%", "35%"]\' hs-size=\'["20%", "15%"]\' ng-repeat="w in num">{{w}}</hs-widget></hs-layout>';
            elem = $compile(elem)(scope);
            scope.$digest();
        }));
        ...
        it('Should have widget width 20%', function() {
        	expect($(widgets[0]).css('width')).toBe('20%'); 
        });
        
        it('Should have widget height 15%', function() {
            expect($(widgets[0]).css('height')).toBe('15%'); 
        });
        
        it('Should have widget left 25%', function() {
        	expect($(widgets[0]).css('left')).toBe('25%'); 
        });
        
        it('Should have widget top 35%', function() {
            expect($(widgets[0]).css('top')).toBe('35%'); 
        });
